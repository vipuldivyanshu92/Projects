...
class RealmHandler(asyncore.dispatcher):
    ...
    def SS_AUTH_LOGON_CHALLENGE(self, data):
        ...
        ## Generate SRP_x
        SRP_authstr = '%s:%s' % (self.config.get('username', ''), self.config.get('password', ''))    # Concatenate USER:PASS
        ...
        SRP_userhash = hashlib.sha1(self.config.get('username', '').upper()).digest()
